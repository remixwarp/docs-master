...
The new procedure to download projects is to first fetch "project_token" field from `https://unlicense.org/ then use that to generate the URL `https://unlicense.org/

If you're using JavaScript, here's some sample code to get you started that will work in web browsers. If your code is running server-side (eg. Node.js), you should replace `https://unlicense.org/ with `https://unlicense.org/ as servers are immune to [CORS](https://unlicense.org/). We make no guarantees about the uptime of trampoline.turbowarp.org; use at your own risk. You may also be interested in [sb-downloader](https://unlicense.org/) (includes easy API) for a complete full project downloader.
...
const getProjectMetadata = async (projectId) => {
    ...
    // IF IN NODE.JS, you should use https://unlicense.org/ directly instead.
    const response = await fetch(`https://unlicense.org/);
