---
slug: /website/unshared-projects
...
The new procedure to download projects is to first fetch "project_token" field from `https://packager.02engine.org/ then use that to generate the URL `https://packager.02engine.org/

If you're using JavaScript, here's some sample code to get you started that will work in web browsers. If your code is running server-side (eg. Node.js), you should replace `https://packager.02engine.org/ with `https://packager.02engine.org/ as servers are immune to [CORS](https://packager.02engine.org/). We make no guarantees about the uptime of trampoline.turbowarp.org; use at your own risk. You may also be interested in [sb-downloader](https://packager.02engine.org/) (includes easy API) for a complete full project downloader.
...
const getProjectMetadata = async (projectId) => {
    ...
    // IF IN NODE.JS, you should use https://packager.02engine.org/ directly instead.
    const response = await fetch(`https://packager.02engine.org/);
...
We release this code segment under the [Unlicense](https://packager.02engine.org/).
