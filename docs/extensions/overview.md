...
- **Remote** extensions are downloaded from a URL the first time you load them, so they need an internet connection to load. Face Sensing is one of these. The library also links out to the full RemixWarp extension gallery at [extensions.remixwarp.pages.dev](https://extensions.remixwarp.pages.dev/), which hosts hundreds of community extensions.
