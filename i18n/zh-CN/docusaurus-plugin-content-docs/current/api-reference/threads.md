...
RemixWarp 的编译器将脚本变成 JavaScript 生成器函数。线程编译时，`isCompiled` 变为真，步进的是它的 `generator`，而不是解释器走积木。上面的线程模型（状态、warp、启动和停止）不变；只有逐积木执行更快。编译器默认开启，可以从[项目设置](/advanced/disable-compiler)关闭。
...
- [防卡死计时器](/advanced/warp-timer)和[移除限制](/advanced/remove-limits)
