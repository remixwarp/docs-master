...
- `mwProjectTheme`（`mw-project-theme.js`）支撑应用嵌入项目中的主题的提示。请参阅[主题](/internals/theming)。
...
容器直接导入这些。`mapStateToProps` 读取切片（或调用选择器），`mapDispatchToProps` 包裹一个导入的 action creator，使组件接收普通的回调。模式请参阅[容器](/internals/containers)。
