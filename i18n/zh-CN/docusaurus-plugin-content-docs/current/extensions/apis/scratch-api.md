...
| `HAT` | `hat` | 有条件地启动堆栈（请参阅[事件和帽子](/extensions/hats)） |
| `EVENT` | `event` | 在事件上启动堆栈；没有函数 |
| `CONDITIONAL` | `conditional` | `如果`/`否则` 风格 C 积木（请参阅[自定义 C 积木](/extensions/custom-c-blocks)） |
...
`STRING`、`NUMBER`、`BOOLEAN`、`COLOR`、`ANGLE`、`MATRIX`、`NOTE`、`COSTUME`、`SOUND`、`IMAGE`。每个接受什么请参阅[处理输入](/extensions/inputs)。
...
Scratch 语义的转换。完整细节在[实用 API 页面](/extensions/apis/utility-apis)。
...
VM 实例。完整的表面请参阅[VM API 页面](/extensions/apis/vm-api)。
...
WebGL 渲染器。请参阅[渲染器 API 页面](/extensions/apis/renderer-api)。
...
非沙箱积木函数接收第二个参数 `util`，只在积木运行时有效（请参阅[非沙箱扩展](/extensions/unsandboxed)）。
...
非沙箱扩展在接触网络或外部世界之前必须请求。为什么以及如何请参阅[非沙箱扩展](/extensions/unsandboxed)。
...
- [音频 API](/extensions/apis/audio-api)
- [实用 API](/extensions/apis/utility-apis)
