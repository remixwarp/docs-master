...
让我们用受支持的 [`compiler.register` API](/extensions/compiled/structure) 构建一个带两个编译报告积木 `square` 和 `power` 的小"Math Utils"扩展。
...
1. 在编辑器中以非沙箱方式加载它（从 `http://localhost:8000/` 或使用"不进入沙箱运行"选项；请参阅[非沙箱扩展](/extensions/unsandboxed)）。
...
[高级编译技巧](/extensions/compiled/advanced)：命令积木、可变参数输入、原始输入和接触运行时状态。
