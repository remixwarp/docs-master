...
- **从 URL 加载**：只有当 URL 恰好以下面一个受信任前缀开头时，它才自动以非沙箱方式运行：
  - `https://extensions.remixwarp.pages.dev/`
  - `https://extensions.turbowarp.org/`
  - `http://localhost:8000/`

  任何其他 URL 都以沙箱方式加载，除非用户手动选择信任它。无法强制任意 URL 以非沙箱方式加载，这是故意的，为了保护用户免受恶意扩展的侵害。

由于您不控制那两个画廊域名，开发期间您从 `http://localhost:8000/` 提供扩展。将您的[本地服务器](/extensions/introduction)指向 8000 端口。它必须恰好是 8000 端口上的 `localhost`；`127.0.0.1` 和 `0.0.0.0` 不会被视为受信任。
...
您的开发者控制台是最好的探索方式：扩展加载后，您可以在那里检查 `Scratch` 和 `vm`。[scratch-vm 源码](/development/project-structure)是存在什么的参考。
...
设备访问存在其他权限检查：`Scratch.canRecordAudio()`、`canRecordVideo()`、`canReadClipboard()`、`canNotify()`、`canGeolocate()`、`canEmbed()`、`canDownload()`。请参阅 [Scratch API 参考](/extensions/apis/scratch-api)。
...
厌倦了硬刷新？让我们设置[一个更好的开发服务器](/extensions/better-development-server)。
