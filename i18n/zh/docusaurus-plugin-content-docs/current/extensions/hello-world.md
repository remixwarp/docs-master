...
这里有一个完整的扩展，它添加一个积木，一个返回"World!"的报告积木。将其保存为 `hello-world.js`，放在您的[开发服务器](/extensions/introduction)可以提供服务的位置（[下载](/example-extensions/hello-world.js)）。
...
| `arguments` | object | 可选。[处理输入](/extensions/inputs)中介绍。 |
...
`HAT` 和 `EVENT` 在[事件和帽子](/extensions/hats)中介绍。`CONDITIONAL` 和 `LOOP`（C 形积木）在[自定义 C 积木](/extensions/custom-c-blocks)中介绍。
