...
编译扩展是一个普通扩展加上一次 `Scratch.vm.exports.compiler.register` 调用。本页介绍该调用的形状。[下一页](/extensions/compiled/first-extension)构建一个完整示例。
...
端到端[构建第一个编译扩展](/extensions/compiled/first-extension)。
