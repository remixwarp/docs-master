        ...
        },
        {
          ...
          arguments: {
            URL: {
              ...
              defaultValue: 'https://extensions.remixwarp.pages.dev/hello.txt'
            }
          }
        }
      ]
...
::::info
在[非沙箱扩展](/extensions/unsandboxed)中，您应该使用 `Scratch.fetch(url)` 而不是这里显示的普通 `fetch(url)`，以便尊重用户的获取权限设置。在沙箱扩展中，普通 `fetch` 没问题。
::::
...
即使不返回 Promise 的积木也不是真正即时的，并且无法接触许多 API。要理解为什么，我们需要谈谈[沙箱](/extensions/sandbox)。
