...
> **âš ï¸ Requirement:** Custom C blocks only work in **unsandboxed** extensions.
...
- `Scratch.BlockType.LOOP` â€?For repeating blocks. The loop is assumed to have exactly one child branch; `branchCount` is implied and not required.
- `Scratch.BlockType.CONDITIONAL` â€?For if/else style blocks. Must specify `branchCount` to indicate how many branches the block controls.
