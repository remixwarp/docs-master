...
讓我們用受支持的 [`compiler.register` API](/building-extensions/compiled/structure) 構建一個帶兩個編譯報告積木 `square` 和 `power` 的小"Math Utils"擴展。
...
1. 在編輯器中以非沙箱方式加載它（從 `http://localhost:8000/` 或使用"不進入沙箱運行"選項；請參閱[非沙箱擴展](/building-extensions/unsandboxed)）。
...
[高級編譯技巧](/building-extensions/compiled/advanced)：命令積木、可變參數輸入、原始輸入和接觸運行時狀態。
