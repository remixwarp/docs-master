...
由於您不控制那兩個畫廊域名，開發期間您從 `http://localhost:8000/` 提供擴展。將您的[本地服務器](/building-extensions/introduction)指向 8000 端口。它必須恰好是 8000 端口上的 `localhost`；`127.0.0.1` 和 `0.0.0.0` 不會被視為受信任。
...
您的開發者控制台是最好的探索方式：擴展加載後，您可以在那裡檢查 `Scratch` 和 `vm`。[scratch-vm 源碼](/contributing/project-structure)是存在什麼的參考。
...
設備訪問存在其他權限檢查：`Scratch.canRecordAudio()`、`canRecordVideo()`、`canReadClipboard()`、`canNotify()`、`canGeolocate()`、`canEmbed()`、`canDownload()`。請參閱 [Scratch API 參考](/building-extensions/apis/scratch-api)。
...
厭倦了硬刷新？讓我們設置[一個更好的開發服務器](/building-extensions/better-development-server)。
