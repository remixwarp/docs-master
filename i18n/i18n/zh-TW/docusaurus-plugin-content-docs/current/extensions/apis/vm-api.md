...
VM（虛擬機）運行項目。[非沙箱擴展](/extensions/unsandboxed)以 `Scratch.vm` 訪問它，積木函數以 `util.runtime` 訪問它（運行時是 `vm.runtime`）。
...
`startHats` 返回啟動的 `Thread` 對象。在運行中的積木內部，請使用 `util.startHats`。請參閱[事件和帽子](/extensions/hats)。
...
- `util.startBranch(index, isLoop)` 運行 C 積木分支（請參閱[自定義 C 積木](/extensions/custom-c-blocks)）。
...
要直接生成 JavaScript（原生速度積木），RemixWarp 通過 `vm.exports.compiler` 暴露受支持的擴展編譯器 API。這是它自己的主題；請參閱[編譯擴展](/extensions/compiled/overview)。
