...
| `HAT` | `hat` | 有條件地啟動堆棧（請參閱[事件和帽子](/extensions/hats)） |
| `EVENT` | `event` | 在事件上啟動堆棧；沒有函數 |
| `CONDITIONAL` | `conditional` | `如果`/`否則` 風格 C 積木（請參閱[自定義 C 積木](/extensions/custom-c-blocks)） |
...
`STRING`、`NUMBER`、`BOOLEAN`、`COLOR`、`ANGLE`、`MATRIX`、`NOTE`、`COSTUME`、`SOUND`、`IMAGE`。每個接受什麼請參閱[處理輸入](/extensions/inputs)。
...
Scratch 語義的轉換。完整細節在[實用 API 頁面](/extensions/apis/utility-apis)。
...
VM 實例。完整的表面請參閱[VM API 頁面](/extensions/apis/vm-api)。
...
WebGL 渲染器。請參閱[渲染器 API 頁面](/extensions/apis/renderer-api)。
...
非沙箱積木函數接收第二個參數 `util`，只在積木運行時有效（請參閱[非沙箱擴展](/extensions/unsandboxed)）。
...
非沙箱擴展在接觸網絡或外部世界之前必須請求。為什麼以及如何請參閱[非沙箱擴展](/extensions/unsandboxed)。
...
- [渲染器 API](/extensions/apis/renderer-api)
- [音頻 API](/extensions/apis/audio-api)
- [實用 API](/extensions/apis/utility-apis)
