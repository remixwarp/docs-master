...
02Engine Packager將 Scratch 或 RemixWarp 項目變成獨立程序：單個 HTML 文件、zip，或 Windows、macOS、Linux 的原生可執行文件。輸出不需要編輯器或互聯網連接即可運行，並且捆綁了 RemixWarp 快速的編譯運行時。
...
請記住，[未共享的 Scratch 項目](/website/unshared-projects)不能通過 ID 加載；請下載 `.sb3` 並改為上傳。

## 常用設置

打包器公開與編輯器相同的大多數運行時選項，包括[自定義 FPS](/website/custom-fps)、[插值](/website/interpolation)、[高質量畫筆](/website/high-quality-pen)和[自定義舞臺大小](/website/custom-stage-size)。它還添加打包器特有的功能：
