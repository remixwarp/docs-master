...
[沙箱擴展](/extensions/sandbox)可以託管在任何通過 HTTPS 提供靜態文件的地方。[GitHub Pages](https://pages.github.com/) 是常見選擇。注意 GitHub 的 `raw.githubusercontent.com` 鏈接**不能**作為擴展 URL；請改用 GitHub Pages（或其他靜態主機）。

一旦它在公開的 HTTPS URL 上，就在編輯器中使用"添加擴展"、"自定義擴展"和該 URL 加載它，或用 `?extension=<url>` 自動加載。

## 非沙箱擴展

[非沙箱擴展](/extensions/unsandboxed)只從受信任的來源自動以非沙箱方式運行（請參閱[非沙箱頁面](/extensions/unsandboxed)上的受信任前綴）。由於您不控制那些畫廊域名，實際可行的選項是：
