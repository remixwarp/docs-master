...
這裡有一個完整的擴展，它添加一個積木，一個返回"World!"的報告積木。將其保存為 `hello-world.js`，放在您的[開發服務器](/extensions/introduction)可以提供服務的位置（[下載](/example-extensions/hello-world.js)）。
...
| `arguments` | object | 可選。[處理輸入](/extensions/inputs)中介紹。 |
...
`HAT` 和 `EVENT` 在[事件和帽子](/extensions/hats)中介紹。`CONDITIONAL` 和 `LOOP`（C 形積木）在[自定義 C 積木](/extensions/custom-c-blocks)中介紹。
