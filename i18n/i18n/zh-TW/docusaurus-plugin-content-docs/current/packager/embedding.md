...
::::info
本頁講述嵌入[RemixWarp 打包器](/packager/overview)的輸出。如果您只是想嵌入共享項目而不打包它，請參閱[嵌入](/advanced/embedding)。
::::
...
因為打包項目只是您託管的文件，嵌入它對未在 Scratch 上共享的項目也有效，與[託管嵌入播放器](/advanced/embedding)不同。

## 另請參閱

- [打包器概覽](/packager/overview)
- [嵌入共享項目](/advanced/embedding)
