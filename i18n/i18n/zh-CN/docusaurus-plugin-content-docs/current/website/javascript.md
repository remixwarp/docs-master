...
- **调试编译器 bug。** 如果脚本在 RemixWarp 中的行为与在 Scratch 中不同，您可以关闭编译器检查是否是编译器的错。请参阅[禁用编译器](/website/disable-compiler)。
- **查看生成的代码**（见下文）。
...
这是真正的编程任务，不是积木。完整指南在[构建扩展](/extensions/introduction)，非沙箱环境的具体细节在[非沙箱扩展](/extensions/unsandboxed)。您可以通过 [`extension` URL 参数](/website/url-parameters#extension) 从 URL 加载扩展：
...
要将项目作为独立 HTML 文件或应用发布，请使用[02Engine Packager](/packager/overview)。打包器将编译后的运行时与您的项目捆绑在一起，使其在任何地方都能运行，无需编辑器。

## 另请参阅

- [补丁扩展](/extensions/patching)
- [禁用编译器](/website/disable-compiler)
- [构建扩展](/extensions/introduction)
