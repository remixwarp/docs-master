...
02Engine Packager将 Scratch 或 RemixWarp 项目变成独立程序：单个 HTML 文件、zip，或 Windows、macOS、Linux 的原生可执行文件。输出不需要编辑器或互联网连接即可运行，并且捆绑了 RemixWarp 快速的编译运行时。
...
请记住，[未共享的 Scratch 项目](/website/unshared-projects)不能通过 ID 加载；请下载 `.sb3` 并改为上传。

## 常用设置

打包器公开与编辑器相同的大多数运行时选项，包括[自定义 FPS](/website/custom-fps)、[插值](/website/interpolation)、[高质量画笔](/website/high-quality-pen)和[自定义舞台大小](/website/custom-stage-size)。它还添加打包器特有的功能：
