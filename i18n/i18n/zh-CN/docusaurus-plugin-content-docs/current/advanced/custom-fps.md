...
帧率（每秒帧数）控制您的脚本每秒运行多少次。Scratch 以 30 FPS 运行。RemixWarp 允许您更改此设置，最常见的是 60 FPS，可以从[编辑器设置](/editor/settings)或 [`fps` URL 参数](/advanced/url-parameters) 中进行设置。
...
如果您只想获得更流畅的运动而不改变游戏速度，请使用[插值](/advanced/interpolation)而不是提高帧率。插值让脚本保持正常速率，并在其间平滑视觉效果。
