...
渲染器（scratch-render）用 WebGL 繪製舞臺。[非沙箱擴展](/extensions/unsandboxed)在 `Scratch.renderer` 訪問它（等價於 `util.runtime.renderer`）。
...
舞臺座標與 Scratch 匹配：`(0, 0)` 是中心，x 向右增加，y 向上增加。在默認舞臺上 x 從 -240 到 240，y 從 -180 到 180，但[舞臺大小可以自定義](/website/custom-stage-size)，因此請讀取 `runtime.stageWidth` / `runtime.stageHeight`，而不是假設。
