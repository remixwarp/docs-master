...
**協作。** 在 Scratch 上共享項目。共享未完成的工作完全可以。如果您想與他人實時編輯，RemixWarp 還內置了完全不依賴 Scratch 的[協作](/user-guide/collaboration)。

**嵌入。** 在 Scratch 上共享項目，或下載 `.sb3` 並使用[02Engine Packager](/packager/overview)將其轉換為可以[嵌入](/packager/embedding)的獨立文件。
...
瀏覽器不能直接調用 `api.scratch.mit.edu`，因為它不發送 [CORS](/website/cors) 頭，因此瀏覽器端代碼需要 CORS 代理；服務器端代碼（Node.js）不受 CORS 約束，可以直接調用。有關處理這一切的現成下載器，請參閱 [sb-downloader](https://github.com/forkphorus/sb-downloader)。

## 另請參閱

- [協作](/user-guide/collaboration)
