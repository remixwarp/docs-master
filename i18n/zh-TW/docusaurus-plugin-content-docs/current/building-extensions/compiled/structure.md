...
編譯擴展是一個普通擴展加上一次 `Scratch.vm.exports.compiler.register` 調用。本頁介紹該調用的形狀。[下一頁](/building-extensions/compiled/first-extension)構建一個完整示例。
...
端到端[構建第一個編譯擴展](/building-extensions/compiled/first-extension)。
