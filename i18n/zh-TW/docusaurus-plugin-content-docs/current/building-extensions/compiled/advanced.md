...
本頁涵蓋[`compiler.register` API](/building-extensions/compiled/structure) 超出普通報告積木的部分：命令積木、字段、可變參數輸入、原始輸入和接觸運行時狀態。這裡的一切都對照 RemixWarp 自己的內置運算積木的編譯方式驗證過。
...
每個編譯積木仍然需要它的解釋器方法（請參閱[您的第一個編譯擴展](/building-extensions/compiled/first-extension)）。它在積木未編譯時運行，因此必須計算相同的結果。當您更改編譯邏輯時，也要更改回退。

## 不要做什麼

編譯器故意很窄，這就是重點。它沒有受支持的 API 用於多遍編譯、將共享輔助函數提升到輸出中、掛鉤編譯器的生命週期，或在編譯時保持跨積木狀態。如果您發現自己想要這些，積木可能太大而不值得編譯；請改為將其寫為普通的[非沙箱積木](/building-extensions/unsandboxed)。

## 另請參閱

- [編譯擴展概覽](/building-extensions/compiled/overview)
- [遺留補丁方法](/building-extensions/compiled/patching)
