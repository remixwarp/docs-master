...
幀率（每秒幀數）控制您的腳本每秒運行多少次。Scratch 以 30 FPS 運行。RemixWarp 允許您更改此設置，最常見的是 60 FPS，可以從[編輯器設置](/editor/settings)或 [`fps` URL 參數](/advanced/url-parameters) 中進行設置。
...
如果您只想獲得更流暢的運動而不改變遊戲速度，請使用[插值](/advanced/interpolation)而不是提高幀率。插值讓腳本保持正常速率，並在其間平滑視覺效果。
