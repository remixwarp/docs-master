...
- **調試編譯器 bug。** 如果腳本在 RemixWarp 中的行為與在 Scratch 中不同，您可以關閉編譯器檢查是否是編譯器的錯。請參閱[禁用編譯器](/advanced/disable-compiler)。
- **查看生成的代碼**（見下文）。
...
這是真正的編程任務，不是積木。完整指南在[構建擴展](/building-extensions/introduction)，非沙箱環境的具體細節在[非沙箱擴展](/building-extensions/unsandboxed)。您可以通過 [`extension` URL 參數](/advanced/url-parameters#extension) 從 URL 加載擴展：
...
要將項目作為獨立 HTML 文件或應用發佈，請使用[RemixWarp 打包器](/packager/overview)。打包器將編譯後的運行時與您的項目捆綁在一起，使其在任何地方都能運行，無需編輯器。
